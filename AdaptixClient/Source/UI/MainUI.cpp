...
MainUI::MainUI()
{
    ...
    projectSettingsAction = new QAction(this);
...
void MainUI::retranslateUi()
{
    ...
    projectSettingsAction->setText(tr("Subscriptions"));
    menuProject->setTitle(tr("Projects"));

    axConsoleAction->setText(tr("AxScript console "));
    scriptManagerAction->setText(tr("Script manager"));
    menuExtensions->setTitle(tr("Extensions"));

    settingsAction->setText(tr("Open settings"));
...
void MainUI::updateTabButton(const int index, const QString& tabName, const bool showButton)
{
    ...
    if (showButton) {
        ...
        folderButton->setToolTip(tr("Open project directory"));
