...
DialogSettings::DialogSettings(Settings* s)
{
    ...
    connect(graphCombo1,    &QComboBox::currentTextChanged, buttonApply, [this](const QString &){buttonApply->setEnabled(true);} );
...
void DialogSettings::createUI()
{
    ...
    languageLabel = new QLabel("Language:", mainSettingWidget);
    ...
    mainSettingLayout->addWidget(languageLabel,     4, 0, 1, 1);
    mainSettingLayout->addWidget(languageCombo,     4, 1, 1, 2);
...
void DialogSettings::onApply() const
{
    ...
    if (settings->data.Language != selectedLang) {
        settings->data.Language = selectedLang;
        LangManager::instance().switchLanguage(selectedLang);
    }
...
void DialogSettings::loadSettings()
{
    ...
    {
        int idx = languageCombo->findData(settings->data.Language);
        languageCombo->setCurrentIndex(idx >= 0 ? idx : 0);
    }
...
{
    setWindowTitle(tr("Adaptix Settings"));

    themeLabel->setText(tr("Main theme: "));
    themeImportBtn->setText(tr("Import"));
    fontFamilyLabel->setText(tr("Font family: "));
    fontSizeLabel->setText(tr("Font size: "));
    graphLabel1->setText(tr("Session Graph version:"));
    languageLabel->setText(tr("Language:"));
    terminalSizeLabel->setText(tr("RemoteTerminal buffer (lines):"));

    consoleGroup->setTitle(tr("Agent Console"));
    consoleSizeLabel->setText(tr("Buffer size (lines):"));
    consoleTimeCheckbox->setText(tr("Print date and time"));
    consoleNoWrapCheckbox->setText(tr("No Wrap mode"));
    consoleAutoScrollCheckbox->setText(tr("Auto Scroll mode"));
    consoleShowBackgroundCheckbox->setText(tr("Show background image"));
    consoleThemeLabel->setText(tr("Console theme:"));
    consoleThemeImportBtn->setText(tr("Import"));

    sessionsGroup->setTitle(tr("Columns"));
    sessionsHealthCheck->setText(tr("Check Health"));
    sessionsLabel1->setText(tr("Sleeptime *"));
    sessionsLabel3->setText(tr("sec"));

    tasksGroup->setTitle(tr("Columns"));

    tabblinkEnabledCheckbox->setText(tr("Enable tab blink"));
    tabblinkGroup->setTitle(tr("Blinking tabs"));

    const QStringList sessionsLabels = {
    ...
    for (int i = 0; i < sessionsCheckCount; ++i)
        sessionsCheck[i]->setText(sessionsLabels[i]);

    const QStringList tasksLabels = {
        tr("Task ID"), tr("Task Type"), tr("Agent ID"), tr("Client"), tr("User"),
        tr("Computer"), tr("Start Time"), tr("Finish Time"), tr("Commandline"),
        tr("Result"), tr("Output")
    };
    for (int i = 0; i < 11; ++i)
        tasksCheck[i]->setText(tasksLabels[i]);

    const QStringList settingsPages = {
        tr("Main settings"), tr("Sessions table"), tr("Tasks table"), tr("Blinking tabs")
    };
    for (int i = 0; i < listSettings->count() && i < settingsPages.size(); ++i)
        listSettings->item(i)->setText(settingsPages[i]);
    labelHeader->setText(listSettings->currentItem() ? listSettings->currentItem()->text() : QString());
