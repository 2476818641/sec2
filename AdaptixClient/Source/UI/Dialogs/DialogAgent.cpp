...
void DialogAgent::onButtonLoad()
{
    ...
    NonBlockingDialogs::getOpenFileName(this, tr("Select file"), baseDir, "JSON files (*.json)",
...
void DialogAgent::onButtonSave()
{
    ...
    NonBlockingDialogs::getSaveFileName(this, tr("Save File"), initialPath, "JSON files (*.json)",
...
void DialogAgent::onBuildMessage(const QString &msg)
{
    ...
    switch (status) {
        ...
        case 4: { // BUILD_LOG_SAVE_FILE
            ...
            QPointer<DialogAgent> safeThis = this;
            NonBlockingDialogs::getSaveFileName(this, tr("Save File"), initialPath, "All Files (*.*)",
                [safeThis, content](const QString& filePath) {
                    if (filePath.isEmpty())
                        return;
                    ...
                    if (safeThis) {
                        safeThis->buildLogOutput->append(QString("<span style='color: #dcdcaa;'>[+]</span> " + tr("File saved: %1")).arg(filePath.toHtmlEscaped()));
                    }
                });
            return;
        }
