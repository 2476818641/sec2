...
void SessionsTableWidget::createUI()
{
    ...
    checkOnlyActive = new QCheckBox(searchWidget);
...
void SessionsTableWidget::onFilterChanged() const
{
    ...
    QString currentType = comboAgentType->currentText();
    if (currentType != tr("All types") && !currentType.isEmpty()) {
...
void SessionsTableWidget::changeEvent(QEvent* event)
{
...
void SessionsTableWidget::retranslateUi()
{
    ...
    if (comboAgentType->count() > 0)
        comboAgentType->setItemText(0, tr("All types"));
    checkOnlyActive->setText(tr("only active"));
}