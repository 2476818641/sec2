...
void AxConsoleWidget::processInput()
{
    ...
    if (!scriptManager) {
        OutputTextEdit->appendColor(tr("Script engine is not available\n"), QColor(COLOR_ChiliPepper));
        return;
    }

    QJSEngine* engine = scriptManager->MainScriptEngine();
    if (!engine) {
        OutputTextEdit->appendColor(tr("Script engine is not initialized\n"), QColor(COLOR_ChiliPepper));
        return;
    }
...
void AxConsoleWidget::retranslateUi()
{
    searchLineEdit->setPlaceholderText(tr("Find"));
    ResetButton->setText(tr("Reset AxScript"));
}

void AxConsoleWidget::changeEvent(QEvent* event)
{
