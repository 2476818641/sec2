...
protected:
    bool filterAcceptsRow(const int row, const QModelIndex &parent) const override {
        ...
        if (!agentFilter.isEmpty() && agentFilter != tr("All agents") && agent != agentFilter)
            return false;

        if (!typeFilter.isEmpty() && typeFilter != tr("All types") && type != typeFilter)
            return false;

        if (!statusFilter.isEmpty() && statusFilter != tr("Any status") && status != statusFilter)
            return false;
